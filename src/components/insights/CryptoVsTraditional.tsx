    ...
    <div className="rounded-xl border border-neutral-800 bg-neutral-900 p-6">
      ...
      <div className="grid grid-cols-3 gap-4 mb-6">
        <div className="rounded-lg p-4 bg-neutral-950 border-l-4 border-orange-400">
          <div className="text-xs text-orange-400 uppercase tracking-widest">Crypto</div>
          <div className="text-xl font-bold text-orange-400 tabular-nums">{formatCAD(current.cryptoCAD)}</div>
          <div className="text-xs text-neutral-400 tabular-nums">{current.cryptoPct.toFixed(1)}% of portfolio</div>
        </div>
        <div className="rounded-lg p-4 bg-neutral-950 border-l-4 border-cyan-400">
          <div className="text-xs text-cyan-400 uppercase tracking-widest">Traditional + RE</div>
          <div className="text-xl font-bold text-cyan-400 tabular-nums">{formatCAD(current.traditionalCAD)}</div>
          <div className="text-xs text-neutral-400 tabular-nums">{(100 - current.cryptoPct).toFixed(1)}% of portfolio</div>
        </div>
        <div className="rounded-lg p-4 bg-neutral-950 border-l-4 border-neutral-500">
          <div className="text-xs text-neutral-400 uppercase tracking-widest">Total</div>
          <div className="text-xl font-bold text-white tabular-nums">{formatCAD(current.cryptoCAD + current.traditionalCAD)}</div>
        </div>
      </div>
      <h4 className="text-xs font-semibold text-neutral-400 uppercase tracking-widest mb-3">What-If Scenarios</h4>
      <div className="h-56">
        <ResponsiveContainer>
      ...
      <div className="grid grid-cols-4 gap-2 mt-4">
        {scenarios.map(s => (
          <div key={s.label} className={`text-center p-2 rounded text-xs ${s.label === "Current" ? "bg-neutral-800 border border-neutral-700" : "bg-neutral-950"}`}>
            <div className="text-neutral-400 uppercase tracking-wider text-xs">{s.label}</div>
