    ...
    <div className="rounded-xl border border-neutral-800 bg-neutral-900 p-6">
      ...
      <div className="grid grid-cols-2 gap-4 mb-6">
        <div className="rounded-lg p-4 bg-neutral-950 border-l-4 border-emerald-400">
          ...
          <div className="text-2xl font-bold text-emerald-400 tabular-nums">{formatCAD(totalCashCAD)} <span className="text-sm font-normal text-neutral-500">CAD</span></div>
        </div>
        <div className="rounded-lg p-4 bg-neutral-950 border-l-4 border-amber-400">
          ...
          <div className="text-2xl font-bold text-amber-400 tabular-nums">+{formatCAD(gain10y)} <span className="text-sm font-normal text-neutral-500">CAD</span></div>
          <div className="text-xs text-neutral-500 tabular-nums">Could grow to {formatCAD(projections[3]?.at10pct)}</div>
        </div>
      </div>
      <div className="h-64">
        <ResponsiveContainer>
          <BarChart data={chartData}>
            ...
            <YAxis tickFormatter={(v) => `$${(v / 1000).toFixed(0)}K`} tick={{ fill: "#737373", fontSize: 11 }} stroke="#404040" />
